...
ただし、これは非常に単純なデモザイク処理であり、実際のカメラではより高度なアルゴリズムが使用されています。(参考: [デモザイクアルゴリズムうんちく | Optical Learning Blog](http://optical-learning-blog.realop.co.jp/?eid=44))単純な平均を取る方法は、エッジ部分で色にじみが発生したり、モアレが生じたりする可能性があります。LibRaw などのライブラリを使用すれば、より高度なデモザイク処理を行うことが可能です。
また、ホワイトバランス補正や色補正などを行っていないため、色の再現性も十分ではありません。先ほど出力した PPM ファイルでも、全体的に緑が強く出てしまっています。
